    ...
    S: NodeStorage + MetadataStorage + Send + Sync + 'static,
{
    ...
    pub fn new(storage: S, order: usize) -> Result<Self> {
        ...
        let tree = BPlusTree::<S>::new(storage, order, fmt)?;
        Ok(Self {
    ...
    pub fn get_inner(&self) -> &SharedBPlusTree<S> {
        &self.inner
    }
}
...
//    pub fn from_tree(tree: BPlusTree<S>) -> Self {
//        Self {
...
impl<'a, S> Iterator for TypedIter<'a, S>
where
    S: NodeStorage + MetadataStorage + Send + Sync + 'static,
{
...
impl<S> DbBuilder<S> {
    ...
    /// Build the **typed** API.
    pub fn build_typed(self) -> Result<TypedDb<S>>
    where
        S: NodeStorage + MetadataStorage + Send + Sync + 'static,
    {
        let fmt = crate::keyfmt::KeyFormat::Raw(crate::keyfmt::raw::RawFormat);
        let tree = BPlusTree::<S>::new(self.storage, self.opts.order, fmt)?;
...
pub type WriteTxnBytes = WriteTxn;
